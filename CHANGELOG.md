...
### 修正
- SKILL.md: セキュリティ監査の開始前にカレントディレクトリの `.git` / remote URL から対象ユーザー・リポジトリを自動検出するよう変更
  - HTTPS・SSH どちらの remote URL 形式にも対応
  - 検出できた場合は内容を提示して確認のみ
  - `.git` なし・remote なし・GitHub 以外の場合は手動入力にフォールバック
- SKILL.md: セキュリティ診断コマンドをグローバル認証の参照のみに限定し、変更を加えないよう明示

### 改善
- README.md: 使い方セクションのセキュリティ監査フローに Step 0（ユーザー・リポジトリ自動検出）を追記
